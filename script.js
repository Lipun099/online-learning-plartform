...
function savePerformance(username, subject, score) {
...
function getPerformance(username) {
  fetch(`http://localhost:5000/performance/${username}`)
    .then(response => response.json())
    .then(data => console.log(data))
    .catch(error => console.error("Error:", error));
}
...
function getBotReply(message) {
  if (message.includes("html")) return "HTML stands for HyperText Markup Language.";
  if (message.includes("css")) return "CSS is used for styling your HTML elements.";
  if (message.includes("js") || message.includes("javascript")) return "JavaScript adds interactivity to your webpage.";
  if (message.includes("hi") || message.includes("hello")) return "Hello! How can I help you today?";
  return "Hmm, I didn't understand that. Try asking about HTML, CSS, or JS!";
}
